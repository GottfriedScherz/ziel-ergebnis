...
export default function VmBericht() {
  ...
  const [vmUsers, setVmUsers] = useState<any[]>([])
  ...
  // Eintragszeilen: Name + zwei VM-Felder
  ...
  useEffect(() => {
    ...
    dbQuery('profiles', `id=eq.${user.id}&select=*`).then(async data => {
      ...
      // VM-User = alle User, deren direkter Betreuer im Subtree des eingeloggten Users liegt
      // Oder: alle User die einen betreuer_id haben (= keine eigene Struktur darunter, Planungsvariante VM)
      // Vereinfacht: alle sichtbaren User mit karrierestufe=1
      let visible: any[]
      if (prof.is_admin) {
        visible = allProfiles.filter((u:any) => u.karrierestufe === 1)
      } else {
        const subtree = getSubtreeIds(prof.id, allProfiles)
        visible = allProfiles.filter((u:any) =>
          (u.id === prof.id || subtree.includes(u.id)) && u.karrierestufe === 1
        )
      }
      setVmUsers(visible)
      if (visible.length > 0) setSelectedUser(visible[0].id)
  ...
  async function loadAllBerichte(prof: any, allProfiles: any[]) {
    let userIds: string[]
    if (prof.is_admin) {
      userIds = allProfiles.filter((u:any) => u.karrierestufe === 1).map((u:any) => u.id)
    } else {
      const subtree = getSubtreeIds(prof.id, allProfiles)
      userIds = allProfiles.filter((u:any) =>
        (u.id === prof.id || subtree.includes(u.id)) && u.karrierestufe === 1
      ).map((u:any) => u.id)
    }
  ...
  async function saveBericht() {
    ...
    setSaving(true)
    const eintraege = rows.filter(r => r.name.trim())
    ...
    if (data.success) {
      ...
      if (profile) loadAllBerichte(profile, allUsers)
    }
  }
  ...
  async function openEdit(bericht: any) {
    const e = eintraege.filter(x => x.vm_bericht_id === bericht.id)
    setSelectedUser(bericht.user_id)
    setMonat(bericht.monat)
    setWoche(bericht.woche)
    setJahr(parseInt(bericht.jahr))
    setRows(e.length > 0 ? e.map((x:any) => ({
      name: x.name || '',
  ...
  function updateRow(i: number, field: string, value: any) {
    setRows(prev => prev.map((r, idx) => idx === i ? {...r, [field]: field === 'name' ? value : Math.max(0, parseInt(value)||0)} : r))
  }
  ...
  const filteredBerichte = berichte.filter(b => {
    ...
    if (b.jahr !== filterJahr && b.jahr !== String(filterJahr)) return false
    return true
  })
  ...
  if (view === 'edit') return (
    <div className="min-h-screen bg-gray-50">
      <nav className="bg-white border-b border-gray-200 px-4 py-3 flex items-center justify-between">
        ...
        <h1 className="font-bold text-gray-800">📋 VM-Bericht {editMode ? 'bearbeiten' : 'erfassen'}</h1>
        <div className="w-20" />
      </nav>
      <div className="max-w-2xl mx-auto px-4 py-6 space-y-4">
        ...
        <div className="bg-white rounded-2xl border border-gray-200 p-4 flex gap-3 flex-wrap">
          <div className="flex flex-col gap-1">
            <label className="text-xs font-semibold text-gray-400 uppercase tracking-wide">VM-Partner</label>
            <select value={selectedUser} onChange={e => setSelectedUser(e.target.value)}
              className="border border-gray-200 rounded-lg px-3 py-2 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-blue-500">
              {vmUsers.map(u => <option key={u.id} value={u.id}>{u.name}</option>)}
            </select>
          </div>
          <div className="flex flex-col gap-1">
        ...
        <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
          <div className="px-4 py-3 border-b border-gray-100 flex items-center justify-between">
            <h3 className="font-semibold text-gray-700 text-sm">Einträge</h3>
          </div>
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <nav className="bg-white border-b border-gray-200 px-4 py-3 flex items-center justify-between">
        ...
        <button onClick={openNew} className="text-sm bg-blue-600 text-white px-3 py-1.5 rounded-lg font-medium hover:bg-blue-700 transition">+ Neu</button>
      </nav>
