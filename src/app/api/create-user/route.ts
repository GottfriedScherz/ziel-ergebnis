  ...
  if (RESEND_KEY) {
    const emailHtml = `
      <div style="font-family: -apple-system, Arial, sans-serif; max-width: 560px; margin: 0 auto; padding: 32px 24px; background: #fff;">
        ...
        <p style="color: #888; font-size: 13px; line-height: 1.5; margin-top: 24px;">
          Der Link ist 24 Stunden gültig. Falls du ihn nicht angefordert hast, kannst du diese E-Mail ignorieren.
        </p>
    ...
    const emailRes = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        ...
        to: email,
        subject: `Willkommen bei Ziel & Ergebnis, ${name}!`,
        html: emailHtml,
      })
    })
    
    const emailData = await emailRes.json()
    console.log('Email send status:', emailRes.status, emailData)
  }
